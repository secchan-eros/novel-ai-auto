 ...
 * @param buffer {Buffer}
 * @returns {{tEXt: {Comment: string}, pHYs: {x: number, y: number, units: RESOLUTION_UNITS}, [string]: true}}
 */
...
async function writeMetadataB(blob, metadata) {
  ...
  const newBuffer = fabric.util.png.writeMetadata(uint8Array, {})
...
function insertMetadata(chunks, metadata) {
  ...
  if (metadata.pHYs) {
    ...
    if (pHYs) {
      ...
      chunks.splice(1, 0, { name: 'pHYs', data })
    }
  }
}
